...
class GameOver(State):
    def __init__(self, window: Union[Surface, SurfaceType],
        ...
        self.final_score = ScoreText(game_over_config['score'],
    ...
    def draw_state(self):
        ...
        self.window.blit(self.verdict.text, (
            self.window.get_width() / 2 - self.verdict.text.get_width() / 2,
            self.final_score.position[1] - self.verdict.text.get_height() - 10))
        self.final_score.set_score(self.received_data['score'])
        y_score = self.final_score.position[1]
        self.final_score.position = (
            self.window.get_width() / 2 - self.final_score.text.get_width() / 2,
            y_score)
