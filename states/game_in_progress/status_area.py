from states.game_in_progress.level_text import LevelText
from states.game_in_progress.next_circle_text import NextCircleText
from states.game_in_progress.circle import Circle

from shared import colors
from shared.score_text import ScoreText
...
def calculate_score(nr_circles_popped):
    if score_calculator['has-popped-before']:
        score_calculator['current-coefficient'] += \
            score_calculator['increment']
    if nr_circles_popped == 0:
        score_calculator['has-popped-before'] = False
        score_calculator['current-coefficient'] = \
            score_calculator['min-coefficient']
        return 0
...
class StatusArea:
    ...
    def __init__(self, status_area_config, window, circle_config):
        ...
        self.score_text = ScoreText(self.score_config,
                                    self.height,
                                    self.padding)
        self.level_text = LevelText(self.level_config,
                                    self.height,
                                    self.padding,
                                    window)
        self.next_circle = Circle(
            circle_config['radius'] * 3 / 4,
            (0, 0),
            colors.DEFAULT
        )
        self.next_circle_text = NextCircleText(
            self.next_config,
            self.height,
            self.score_text.position[0] + self.score_text.text.get_width(),
            self.level_text.position[0],
            self.next_circle)
