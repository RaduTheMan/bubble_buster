...
class NextCircleText(Text):
    ...
    def __init__(self,
        ...
        self.__adjust_position(height, lower_width, upper_width)

    def __adjust_position(self, height, lower_width, upper_width):
        x = (lower_width + upper_width) / 2 - \
            (self.text.get_width() + 2 * self.next_circle.radius) / 2
        y = height / 2 - self.text.get_height() / 2
        self.position = (x, y)
        self.next_circle.position = (
            x + self.text.get_width() + self.next_circle.radius,
            height / 2
        )
