...
from states.game_in_progress.status_area import calculate_score
from states.game_in_progress.lee import Lee
from states.game_in_progress.neighbour_conditions import \
    is_neighbour_same_color, is_neighbour_circle

from shared import colors
from shared.colors import table


class Level:

    def __init__(self, level_config: str,
                 window: Union[Surface, SurfaceType],
                 radius,
                 status_area, min_group_length):
        ...
        self.directions = [(-self.radius, -(2 * self.radius - 5)),
                           (self.radius, -(2 * self.radius - 5)),
                           (2 * self.radius, 0),
                           (self.radius, 2 * self.radius - 5),
                           (-self.radius, 2 * self.radius - 5),
                           (-2 * self.radius, 0)]
    ...
    def __build_board(self):
        ...
        for level_line in self.level_lines:
            current_position = (2 * self.radius, current_height) \
                if len(level_line) == self.max_nr_circles_line else \
                (3 * self.radius, current_height)

            last_length = self.max_nr_circles_line \
                if len(level_line) == self.max_nr_circles_line \
                else self.max_nr_circles_line - 1

            for color in level_line:
                if color in table.keys():
                    self.board[current_position] = Circle(self.radius,
                                                          current_position,
                                                          table[color])
                    ...
                    self.board[current_position] = EmptyElement(
                        current_position)
                current_position = current_position[
                                       0] + 2 * self.radius, current_height
        ...
        while current_height <= self.available_width + 2 * self.radius - 5:
            ...
            current_position = (2 * self.radius, current_height) \
                if last_length == self.max_nr_circles_line else \
                (3 * self.radius, current_height)
    ...
    def get_base_elements(self, type_element):
        ...
        base_positions = [position for position in positions if
    ...
    def get_front_elements(self, type_element):
        ...
        front_positions = [position for position in positions if
    ...
    def provide_colors(self):
        available_colors = list(self.available_colors.keys())
        current_color_index = random.randint(0, len(available_colors) - 1)
        next_color_index = random.randint(0, len(available_colors) - 1)
        return {
            'current-color': table[available_colors[current_color_index]],
            'next-color': table[available_colors[next_color_index]]
        }

    def provide_next_color(self):
        available_colors = list(self.available_colors.keys())
        next_color_index = random.randint(0, len(available_colors) - 1)
        return table[available_colors[next_color_index]]
    ...
    def __get_position_shot_circle(self, collision_position, shooting_circle):
        ...
        empty_neighbours.sort(
            ...
            geometry.get_distance(neighbour.position, shooting_circle.position))

        element_to_update = empty_neighbours[0]
        self.board[element_to_update.position] = Circle(
            shooting_circle.radius,
    ...
    def pop_circles(self, starting_circle):
        ...
        if len(to_pop_circles) >= self.min_group_length:
            ...
            for circle_position in to_pop_circles.keys():
                self.update_available_colors(self.board[circle_position],
                                             "remove")
            ...
            for key, value in self.board.items():
                if isinstance(value,
                              Circle) and key not in algorithm.visited.keys():
    ...
    def detect_collision(self, shooting_circle):
        ...
        for position in positions:
            if isinstance(self.board[position], Circle):
                distance = geometry.get_distance(position,
                                                 shooting_circle.position)
                if distance <= self.board[position].radius + \
                        shooting_circle.radius:
