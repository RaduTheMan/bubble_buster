...
class Layout:
    ...
    def draw_buttons(self):
        ...
        x = self.window.get_width() / 2 - \
            self.buttons[0].background.get_width() / 2
        y = self.window.get_height() / 2 - \
            self.buttons[0].background.get_height() / 2
        first_position = (x, y)
        self.drawed_buttons_rects = []
        self.drawed_buttons_rects.append(
            self.window.blit(self.buttons[0].background, first_position)
        )
        for i in range(1, nr_buttons):
            ...
            self.drawed_buttons_rects.append(self.window.blit(
